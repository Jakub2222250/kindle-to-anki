#!/usr/bin/env python3
"""
Collocation Evaluation Harness - Tests Collocation generation across models, runtimes, and prompts.
...
def discover_languages() -> List[str]:
    ...
    languages = []
    for corpus_file in FIXTURES_DIR.glob("collocation_corpus_*.jsonl"):
        lang = corpus_file.stem.replace("collocation_corpus_", "")
...
def load_test_corpus(source_lang: str) -> List[TestCase]:
    ...
    corpus_path = FIXTURES_DIR / f"collocation_corpus_{source_lang}.jsonl"
...
def run_evaluation(
    ...
    """Run Collocation evaluation for a specific configuration."""
    ...
    runtime_config = RuntimeConfig(
        ...
        target_language_code="en",  # Not used for collocations but required
        prompt_id=prompt_id,
    )
    ...
    for tc, output in zip(test_cases, collocation_outputs):
        has_output = bool(output.collocations and len(output.collocations) > 0)
        if has_output:
            successful += 1
    ...
    eval_run = EvalRun(
        ...
        runtime_id="chat_completion_collocation",
        model_id=model_id,
        prompt_id=prompt_id,
...
def save_eval_run(eval_run: EvalRun, session_dir: Optional[Path] = None):
    ...
    filename = f"collocation_{eval_run.source_lang}_{eval_run.model_id}{prompt_suffix}.json"
...
def print_summary(eval_run: EvalRun):
    ...
    print("\n" + "=" * 80)
    print("COLLOCATION EVALUATION SUMMARY")
    print("=" * 80)
    ...
    # Show all results with collocations
    print("\nRESULTS:")
    for r in eval_run.results:
        status = "✓" if r.has_output else "✗"
        print(f"\n  {status} [{r.uid}] {r.lemma} ({r.pos})")
        collocations_str = ", ".join(r.collocations) if r.collocations else "(none)"
        print(f"    Collocations: {collocations_str}")
...
def interactive_evaluation():
    ...
    print("\n" + "=" * 60)
    print("COLLOCATION EVALUATION HARNESS - Interactive Mode")
    print("=" * 60)
    ...
    collocation_prompts = list_prompts("collocation")
    all_prompts = ["(default)"] + collocation_prompts
...
def test_matrix_evaluation():
    ...
    collocation_prompts = list_prompts("collocation")
    print(f"Available Collocation prompts: {collocation_prompts}")

    run_matrix_evaluation(
        models=MODELS,
        source_langs=None,  # Auto-discover all languages
        prompt_ids=[None] + collocation_prompts,
    )
