#!/usr/bin/env python3
"""
Hint Evaluation Harness - Tests Hint generation across models, runtimes, and prompts.
...
def discover_languages() -> List[str]:
    ...
    languages = []
    for corpus_file in FIXTURES_DIR.glob("hint_corpus_*.jsonl"):
        lang = corpus_file.stem.replace("hint_corpus_", "")
...
def load_test_corpus(source_lang: str) -> List[TestCase]:
    ...
    corpus_path = FIXTURES_DIR / f"hint_corpus_{source_lang}.jsonl"
...
def run_evaluation(
    ...
    """Run Hint evaluation for a specific configuration."""
    ...
    for tc, output in zip(test_cases, hint_outputs):
        has_output = bool(output.hint and output.hint.strip())
        if has_output:
            successful += 1
    ...
    eval_run = EvalRun(
        ...
        runtime_id="chat_completion_hint",
        model_id=model_id,
        prompt_id=prompt_id,
...
def save_eval_run(eval_run: EvalRun, session_dir: Optional[Path] = None):
    ...
    filename = f"hint_{eval_run.source_lang}_{eval_run.target_lang}_{eval_run.model_id}{prompt_suffix}.json"
...
def print_summary(eval_run: EvalRun):
    ...
    for r in eval_run.results:
        ...
        print(f"    Hint: {r.hint if r.hint else '(empty)'}")
    ...
    for (source_lang, target_lang), lang_runs in runs_by_lang.items():
        ...
        for result in first_run.results:
            ...
            for run in lang_runs:
                ...
                if r:
                    hint = r.hint if r.hint else "(empty)"
                    if len(hint) > 80:
                        hint = hint[:77] + "..."
                    status = "✓" if r.has_output else "✗"
                    ...
                    print(f"  │  {status} ({config_label:30}): {hint}")
...
def interactive_evaluation():
    ...
    print("\n" + "=" * 60)
    print("HINT EVALUATION HARNESS - Interactive Mode")
    print("=" * 60)
    ...
    hint_prompts = list_prompts("hint")
    all_prompts = ["(default)"] + hint_prompts
...
def test_matrix_evaluation():
    ...
    hint_prompts = list_prompts("hint")
    print(f"Available Hint prompts: {hint_prompts}")

    run_matrix_evaluation(
        models=MODELS,
        source_langs=None,  # Auto-discover all languages
        prompt_ids=[None] + hint_prompts,
    )
